...
public class InGameState extends GameState implements ContactListener {
	...
	public void render() {
		...
		sb.append("Vel: " + Ut.nf(lander.body.getLinearVelocity().len()) + " (" + Ut.nf(lander.body.getLinearVelocity().x) + ", " + Ut.nf(lander.body.getLinearVelocity().y) + ") m/s\n");
		sb.append("Vel +1s: " + Ut.nf(lander.ghost1Velocity.len()) + " (" + Ut.nf(lander.ghost1Velocity.x) + ", " + Ut.nf(lander.ghost1Velocity.y) + ") m/s\n");
		sb.append("Gravity: " + Ut.nf(world.getGravity().y) + "m/s/s\n");

		sb.append("\nEngines: " + (int)(lander.thrusterPower.x*100) + "%, " + (int)(lander.thrusterPower.y*100) + "%\n");

		sb.append("Fuel: " + Ut.nf(lander.fuel) + "s @ 50%\n");
		...
		if (lander.maxGroundSpeed > 0){
			...
			Assets.mono13.draw(batch, "Est Contact Speed: " + Ut.nf(lander.maxGroundSpeed) + "m/s", 160, 173);
